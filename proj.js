...
const url='mongodb://localhost/hospitalManagement';
...
app.get('/Hospital',middleware.checkToken,(req,res)=>{
    ...
    const data=db.collection("hospitaldetails").find().toArray().then(result=>res.send(result));
});
app.get('/Hospital',middleware.checkToken,(req,res)=>{
    const name=req.query.name;
    console.log("fetching hospital details");
    const data=db.collection("hospitaldetails").find({"name":name}).toArray().then(result=>res.send(result));
});
app.get('/Ventilator',(req,res)=>{
    console.log("fetching ventilator details");
    const data=db.collection("ventilatordetails").find().toArray().then(result=>(res.send(result)));
});
app.post('/addventilator',(req,res)=>{
    ...
    db.collection("ventilatordetails").insertOne({"name":name,"hId":hId,"ventilatorId":ventilatorId,"status":status});
...
app.delete('/deleteventilator',middleware.checkToken,(req,res)=>{
    ...
    db.collection("ventilatordetails").deleteOne({"ventilatorId":ventilatorId});
...
app.put('/updateVentilator',middleware.checkToken,(req,res)=>{
    ...
    db.collection("ventilatordetails").updateOne({"ventilatorId":ventilatorId},{$set:{"status":status}});
    console.log("updated successufully");
    res.send("updation successful");
});
app.post('/searchventilatorsbystatus',(req,res)=>{
    const status="occupied"//req.query.status;
    console.log(status);
...
app.post('/searchventilatorsbyname',(req,res)=>{
    ...
    var ventilatordetails=db.collection("ventilatordetails").find({"name":new RegExp(name,'i')}).toArray().then(function(result){
        if(result.length==0)
